<?php
/**
 * ApiController.php - Timerecord Api Controller
 *
 * Main Controller for Timerecord Api
 *
 * @category Controller
 * @package Timerecord
...
class ApiController extends CoreController {
    ...
    public function indexAction() {
        ...
        $aReturn = ['state'=>'success','message'=>'Welcome to onePlace Timerecord API'];
    ...
    /**
     * List all Entities of Timerecords
     *
     * @return bool - no View File
     * @since 1.0.0
     */
    public function listAction() {
        ...
        # Check license
        if(!$this->checkLicense('timerecord')) {
            $aReturn = ['state'=>'error','message'=>'no valid license for timerecord found'];
            echo json_encode($aReturn);
            return false;
        }
        ...
        foreach($aLangs as $sLoadLang) {
            if(file_exists(__DIR__.'/../../../oneplace-translation/language/'.$sLoadLang.'.mo')) {
                $translator->addTranslationFile('gettext', __DIR__.'/../../../oneplace-translation/language/'.$sLang.'.mo', 'timerecord', $sLoadLang);
            }
        }
        ...
        $aFields = $this->getFormFields('timerecord-single');
        $aFieldsByKey = [];
        ...
        # Get All Timerecord Entities from Database
                        ...
                        switch($oField->type) {
                            case 'multiselect':
                                ...
                                foreach($oTags as $oTag) {
                                    $aTags[] = ['id'=>$oTag->id,'label'=>$translator->translate($oTag->text,'timerecord',$sLang)];
                                }
                            ...
                            case 'select':
                                ...
                                if($oTag) {
                                    if (property_exists($oTag, 'tag_value')) {
                                        $aPublicItem[$oField->fieldkey] = ['id' => $oTag->id, 'label' => $translator->translate($oTag->tag_value,'timerecord',$sLang)];
                                    } else {
                                        $aPublicItem[$oField->fieldkey] = ['id' => $oTag->getID(), 'label' => $translator->translate($oTag->getLabel(),'timerecord',$sLang)];
                                    }
                                }
                                break;
                            case 'text':
                            case 'date':
                            case 'textarea':
                                $aPublicItem[$oField->fieldkey] = $translator->translate($oItem->getTextField($oField->fieldkey),'timerecord',$sLang);
                                break;
                            default:
                                break;
                        }
    ...
    /**
     * Get a single Entity of Timerecord
     *
     * @return bool - no View File
     * @since 1.0.0
     */
    public function getAction() {
        ...
        # Get Timerecord ID from route
        $iItemID = $this->params()->fromRoute('id', 0);

        # Try to get Timerecord
        try {
            $oItem = $this->oTableGateway->getSingle($iItemID);
        } catch (\RuntimeException $e) {
            # Display error message
            $aReturn = ['state'=>'error','message'=>'Timerecord not found','oItem'=>[]];
            echo json_encode($aReturn);
            return false;
        }

        # Print Entity
        $aReturn = ['state'=>'success','message'=>'Timerecord found','oItem'=>$oItem];
        echo json_encode($aReturn);
